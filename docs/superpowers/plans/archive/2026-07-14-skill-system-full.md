...
> **Archived:** Full plan preserved for history. Current summary: [../2026-07-14-skill-system.md](../2026-07-14-skill-system.md). Module paths are now `imperial.ts` / `vanguard.ts`; migrations `006_player_equipment.sql` / `007_skill_unlocks.sql`.

> **Status:** Implemented. Path IDs renamed to `imperial`/`knight`/`vanguard` — see `005_imperial_skill_paths.sql`. SP-gated per-skill unlocks added in `007_skill_unlocks.sql`.
...
| `src/server/db/schema/006_skill_unlocks.sql` | `player_skill_unlocks` (SP-gated unlock) |
...
| `src/server/db/skillUnlocks.ts` | Unlock CRUD + SP deduct |
| `src/engine/skills/skillUnlock.ts` | SP cost, unlock validation |
